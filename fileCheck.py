...
def check_valid_category(category_input, type_str):
    ...
    if type_str == 'I':
        ...
        if standard_name.lower() == input_lower:
            return True
        for s in synonyms:
...
def check_ledgerfile(ledgers):
    ...
    try:
        ...
        # (드물게) 시스템 날짜를 가져오지 못할 경우를 대비
        print(f"시스템 날짜를 가져오는 데 실패했습니다: {e}")
        # 날짜 검증을 안전하게 실패 처리하기 위해 먼 미래의 날짜로 설정
        today = datetime.date(9999, 12, 31)
    ...
    for line_num, line in enumerate(ledgers, 1):
        ...
        # 6. Payment 검사 (외부 함수)
        if not check_valid_payment(payment_str):
            print("payment 오류")
            return line_num
    ...
    for line in users:
        ...
        try:
            with open(ledger_file_path, 'r', encoding='utf-8') as f:
                ledgers = [line.strip() for line in f if line.strip()]              
        except Exception as e:
